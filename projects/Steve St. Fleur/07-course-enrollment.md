...
- **AI-Assisted Catalog Blurb:**
  - Add an endpoint that calls an LLM (e.g., OpenAI) and returns a Pydantic-validated structured response. For this theme, that could be generating a course catalog blurb (one-paragraph overview, three learning outcomes, target audience) from an instructor-provided list of learning objectives, returning a structured response ready for the public catalog.
- **Rate Limiting:**
