...
### Make sure you use a different folder and virtual environment than the one for lecture.

## Common Gotchas

| Symptom | Fix |
|---------|-----|
| `TypeError: object of type 'generator' has no len()` | Generators don't have a length — they're consumed as you iterate. If you need a count, `sum(1 for _ in gen)`. If you need to iterate twice, materialize: `items = list(gen)`. |
| Generator returns nothing on second iteration | Generators are one-shot — once exhausted, they yield nothing. To re-iterate, call the generator FUNCTION again (`stream_incidents()` not `stream_incidents`). |
| `tracemalloc.get_traced_memory()` returns `(0, 0)` | You forgot to call `tracemalloc.start()` first. Order matters: start → run → measure → stop. |
| Memory delta between list-comp and generator looks tiny | At 30 records, generator overhead (frame state) outweighs list allocation. The real win shows at 10K+ records. The lab's prose acknowledges this. |
| `chunk()` returns the wrong number of batches | The final partial batch is easy to forget. After the loop, check whether `batch` still has items and yield it if so. |
...
2. Load incidents and make a global variable `NOW = datetime(2026, 5, 20, tzinfo=timezone.utc)` for deterministic output.
...
3. Define `stream_incidents()` as a generator function (NOT a list) that yields one incident at a time from `incidents.json`. Make sure your `chunk()` handles it as a true stream — i.e., your code does NOT call `list()` on the input first.

4. Test against four scenarios:
...
5. Print the last column for each, showing your `chunk()` produces it.
...
- **`StopIteration` raised unexpectedly inside your generator** — never `raise StopIteration` manually inside a generator function. Just `return` (or fall off the end). Manual `StopIteration` is silently treated as a bug since PEP 479.
- **`measure()` shows `peak=0`** — `tracemalloc.start()` resets the counter; if your function does its work BEFORE you start tracking, you'll measure nothing. Start, then call.
- **`chunk()` returns `[[1, 2, 3, 4, 5]]` for `chunk(range(10), 5)`** — you're returning the list, not yielding chunks one at a time. Use `yield batch`, not `return batch`.
